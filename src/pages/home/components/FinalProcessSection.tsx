...
export default function FinalProcessSection() {
  const steps = [
    ...
    },
    {
      ...
      description: "Báo cáo định kỳ, chăm sóc khách hàng & duy trì chất lượng dịch vụ."
    }
  ];
    ...
    <div className="md:px-[40px] px-[16px] md:py-[80px] py-[40px] max-w-screen-2xl mx-auto relative">
      ...
      <div className="pt-[30px]">
        <div className="flex flex-shrink-0 items-center gap-3">
          ...
          <span className="text-[16px] font-medium leading-[1.5] text-gray-600">Quy trình hợp tác đơn giản</span>
        </div>
        ...
        <div className="flex flex-col lg:flex-row pt-[30px] gap-[40px] lg:gap-[80px]">
          ...
          <div className="flex flex-col gap-[40px] flex-1">
            <div className="text-[18px] text-gray-600">
              Chất lượng dịch vụ và trải nghiệm du lịch của TTP Travel không chỉ là lời hứa - mà là cam kết thực tế.
            </div>
            ...
            <div>
              <button className="h-[54px] px-[5px] rounded-full text-black flex gap-[14px] items-center relative group bg-gray-100">
                ...
                <span className="pr-[20px] z-10 group-hover:text-white group-focus:text-white transition-colors font-semibold whitespace-nowrap">Đặt tour ngay</span>
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 sm:gap-[40px] pt-[0px] sm:pt-[80px]">
          {steps.map((step, index) => (
            <div key={index} className="flex sm:flex-col gap-[16px] group cursor-pointer">
              ...
              <div className="flex sm:flex-col gap-[16px] pt-[20px] sm:pt-0 sm:pl-[6px] pl-0 pb-[16px] sm:pb-0">
                <div className="text-black sm:text-[32px] text-[20px] font-semibold leading-[1.2]">{step.number}</div>
                <div className="flex flex-col gap-[16px]">
                  <div className="text-black sm:text-[24px] text-[20px] font-semibold leading-[1.2] lg:min-h-[78px]">{step.title}</div>
                  <div className="text-gray-600 sm:text-[18px] text-[16px] leading-[1.5]">{step.description}</div>
                </div>
              </div>
            </div>
          ))}
